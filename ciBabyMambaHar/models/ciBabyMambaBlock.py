"""
CI-BabyMamba-HAR Block - Weight-Tied Bidirectional SSM with Expanded Memory

============== FROZEN ARCHITECTURE SPECIFICATION ==============

Evolution from BabyMamba-Crossover-BiDir to CI-BabyMamba-HAR.
Key change: d_state increased from 8 to 16 to capture nuanced activities
in datasets like PAMAP2 (12 classes) and SKODA (11 classes).

Design Philosophy:
- Weight-Tied Bidirectional: Same SSM params theta for both directions
- 4 Layers Deep: Matches TinierHAR/LightDeepConvLSTM depth
- Expanded Memory: d_state=16 for nuanced activity recognition
- Efficient Width: d_model=24 to fit parameter budget
...
Mathematical Formulation:
    Forward:  h_fwd = SSM_theta(x)
    Backward: h_bwd = SSM_theta(flip(x))  # Same weights theta!
    Fusion:   h_out = h_fwd + flip(h_bwd)

Per-Layer Parameters with d_state=16 (~6,336 each):
    - inProj: d_model -> 2*d_inner = 24 -> 96 = 2,304
    - conv1d: d_inner groups, k=4 = 48*4 = 192
    - xProj: d_inner -> dt_rank + 2*d_state = 48 -> 34 = 1,632
    - dtProj: dt_rank -> d_inner = 2 -> 48 + 48 = 144
    - A_log: d_inner x d_state = 48 x 16 = 768
    - D: d_inner = 48
    - outProj: d_inner -> d_model = 48 -> 24 = 1,152
    - norm: 2 x d_model = 48 x 2 = 96
    Total per layer: ~6,336 params

Why This Beats Previous Architecture:
- d_state=16 captures nuanced temporal patterns for 12-class PAMAP2
- Weight-tied bidirectional for full temporal context
- Combined with CI-Stem for noise isolation
- Combined with Gated Attention for transient event spotlighting
"""
...
# SimpleMambaBlock now points to the bidirectional block
SimpleMambaBlock = WeightTiedBiDirMambaBlock


class SEBlock(nn.Module):
    """Legacy: Squeeze-and-Excitation Block. NOT used in CiBabyMambaHar."""
...
class BabyMambaBlock(nn.Module):
    """Legacy: Old BabyMamba Block. Kept for backward compatibility."""
    
    def __init__(self, dModel: int, dState: int = 16, expand: int = 2, useSE: bool = True):
        super().__init__()
        self.useSE = useSE
        
        if MAMBA_AVAILABLE:
            self.mamba = Mamba(d_model=dModel, d_state=dState, d_conv=4, expand=expand)
        else:
            self.mamba = PureSelectiveScan(dModel=dModel, dState=dState, dConv=4, expand=expand)
        
        if useSE:
            self.se = SEBlock(dModel, reduction=4)
        
        self.norm = nn.LayerNorm(dModel)
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        xT = x.transpose(1, 2)
        xMamba = self.mamba(xT).transpose(1, 2)
        if self.useSE:
            xMamba = self.se(xMamba)
        return self.norm((x + xMamba).transpose(1, 2)).transpose(1, 2)


class BiDirectionalMambaBlock(nn.Module):
    """Legacy: Old Bi-Directional Mamba Block. Kept for backward compatibility."""
    
    def __init__(self, dModel: int = 24, dState: int = 16, expand: int = 2,
                 useSE: bool = True, dropout: float = 0.1):
        super().__init__()
        self.useSE = useSE
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        residual = x
        xT = x.transpose(1, 2)
        xFwd = self.mamba(xT).transpose(1, 2)
        xFlip = x.flip(dims=[-1]).transpose(1, 2)
        xBwd = self.mamba(xFlip).transpose(1, 2).flip(dims=[-1])
        xBidir = (xFwd + xBwd) * 0.5
        if self.useSE:
            xBidir = self.se(xBidir)
        if self.dropout:
            xBidir = self.dropout(xBidir)
        return self.norm((residual + xBidir).transpose(1, 2)).transpose(1, 2)


class RecursiveBiDirectionalBlock(nn.Module):
    """Legacy: Recursive Bi-Directional Block. Kept for backward compatibility."""
    
    def __init__(self, dModel: int, dState: int = 16, expand: int = 2,
                 nPasses: int = 3, bidirectional: bool = True, useSE: bool = True):
        super().__init__()
        self.nPasses = nPasses
        self.bidirectional = bidirectional
        
        self.sharedBlock = BabyMambaBlock(dModel=dModel, dState=dState, expand=expand, useSE=useSE)
        
        if bidirectional:
            self.biDirScale = nn.Parameter(torch.ones(2) * 0.5)
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        for _ in range(self.nPasses):
            if self.bidirectional:
                xFwd = self.sharedBlock(x)
                xBwd = self.sharedBlock(x.flip(dims=[-1])).flip(dims=[-1])
                x = self.biDirScale[0] * xFwd + self.biDirScale[1] * xBwd
            else:
...
class SEBlock(nn.Module):
    """Legacy: Squeeze-and-Excitation Block. NOT used in CI-BabyMamba-HAR."""
...
class BabyMambaBlock(nn.Module):
    """Legacy: BabyMamba Block with SE. Kept for backward compatibility."""
...
class BiDirectionalMambaBlock(nn.Module):
    """Legacy: Bi-Directional Mamba Block. Kept for backward compatibility."""
